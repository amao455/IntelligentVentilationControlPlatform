...
// Canvas 2D 降级方案 - 始终可用
export function HomeObjBackground({ paused = false, rotationSpeed = 0.06, opacity = 0.58, brightness = 1 }: HomeObjBackgroundProps) {
  ...
  useEffect(() => {
    ...
    const particles: Array<{ x: number; y: number; z: number; vz: number }> = [];
    for (let i = 0; i < 80; i++) {
      particles.push({
        x: Math.random() * canvas.width,
        y: Math.random() * canvas.height,
        z: Math.random() * 1000,
        vz: paused ? 0 : Math.random() * rotationSpeed * 25 + 0.5,
      });
    }

    let animationId: number;
    const animate = () => {
      ctx.fillStyle = `rgba(10, 25, 41, ${0.12 * opacity})`;
      ...
      if (!paused) {
        particles.forEach((p) => {
          ...
          gradient.addColorStop(0, `rgba(120, 180, 240, ${alpha * 0.4})`);
          gradient.addColorStop(0.5, `rgba(120, 180, 240, ${alpha * 0.2})`);
